class ImportSubscribersJob < ApplicationJob
    ...
    ActsAsTenant.with_tenant(account) do
      ...
      rows.each_with_index do |row, idx|
        ...
        begin
          ...
          # Auto-generate ppoe_username / ppoe_password per the tenant's
          # subscriber settings — mirrors the same logic used for
          # single-subscriber creation in SubscribersController#create.
          # Only generate a ref_no if the subscriber doesn't already have
          # one, so re-importing (upsert by phone) doesn't clobber an
          # existing identifier on update.
          if use_autogenerated_number_as_ppoe_username || use_autogenerated_number_as_ppoe_password
            subscriber.ref_no ||= SecureRandom.random_number(10**4)

            if prefix.present? && minimum_digits.to_i > 0
              auto_generated_no = "#{prefix}#{subscriber.ref_no.to_s.rjust(minimum_digits.to_i, '0')}"
              subscriber.ppoe_username = auto_generated_no if use_autogenerated_number_as_ppoe_username
              subscriber.ppoe_password = auto_generated_no if use_autogenerated_number_as_ppoe_password
            end
          end
  ...
  def parse_file(path, filename)
    ...
    if ext == '.csv'
      # 'bom|utf-8' strips a leading BOM if present (common from Excel
      # exports), which otherwise corrupts the *first* header's key.
      # header_converters normalizes every header the same way the xlsx
      # branch below already does — strip whitespace, downcase, spaces to
      # underscores — so row['name'] / row['phone'] reliably match
      # regardless of how the source file capitalized or spaced its
      # headers (this was the actual cause of every row being flagged
      # "missing name or phone" despite the data being present).
      CSV.read(path, headers: true, encoding: 'bom|utf-8',
        header_converters: ->(h) { h&.to_s&.strip&.downcase&.gsub(/\s+/, '_') }
      ).map(&:to_h)
