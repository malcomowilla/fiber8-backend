...
# app/jobs/send_sms_hotspot_job.rb
class SendSmsHotspotJob < ApplicationJob
  queue_as :default
  ...
  def perform(voucher_code, data, checkout_request_id)
    ...
    ActsAsTenant.with_tenant(voucher.account) do
      ...
      return if voucher.reload.sms_sent_at_voucher.present?

      HotspotMpesaRevenue.find_or_create_by(
        reference: data["TransID"]
      ) do |r|
        r.amount         = data["TransAmount"]
        r.voucher        = voucher_code
        r.payment_method = "Mpesa"
        r.time_paid      = data["TransTime"]
        r.account_id     = voucher.account_id
        r.name           = data["FirstName"]
        r.status         = "Completed"
      end

      HotspotSmsService.send_voucher_sms(voucher, ActsAsTenant.current_tenant)
    end
  end
end
