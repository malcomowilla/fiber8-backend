...
def check_sms_already_verified
  admin = SystemAdmin.find_by(phone_number: params[:phone_number]) || SystemAdmin.find_by(phone_number: params[:phone_number2])
  if admin
...
render json: { error: 'System Admin not found' }, status: :unauthorized
  ...
  def login
    @user = SystemAdmin.find_by(
     SystemAdmin.find_by(system_admin_phone_number: params[:phone_number]))
