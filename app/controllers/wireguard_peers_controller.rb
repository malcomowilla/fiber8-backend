...
class WireguardPeersController < ApplicationController
  ...
  def create
    ...
    if @wireguard_peer.save
      RemoteWireguardExecutor.route_add(@wireguard_peer.private_ip)
  ...
  def update
    ...
    ActiveRecord::Base.transaction do
      RemoteWireguardExecutor.route_del(old_ip) if old_ip.present?

      route_result = RemoteWireguardExecutor.route_add(new_ip)
      raise "Failed to add route for #{new_ip}" if route_result.nil?
    ...
    begin
      RemoteWireguardExecutor.route_del(new_ip) if new_ip.present?
    rescue
      # ignore
    end

    begin
      RemoteWireguardExecutor.route_add(old_ip) if old_ip.present?
    rescue
      # ignore
    end
  ...
  def destroy
    ...
    private_ip = @wireguard_peer.private_ip
    @wireguard_peer.destroy!
    RemoteWireguardExecutor.route_del(private_ip)
