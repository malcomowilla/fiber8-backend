...
def create
  ...
  def update
    ...
    old_ip = @wireguard_peer.private_ip
    new_ip = clean_private_ip(
      params.dig(:wireguard_peer, :private_ip)
    )

    if new_ip.blank?
      render json: {
        error: "Private IP cannot be blank"
      }, status: :unprocessable_entity

      return
    end

    @wireguard_peer.private_ip = new_ip
    ...
    begin
      # Remove old route
      delete_route(old_ip) if old_ip.present?
      ...
      unless @wireguard_peer.save
        # Restore old route if database save fails
        delete_route(new_ip)
        add_route(old_ip) if old_ip.present?
      ...
      Rails.logger.error(
        "WireGuard route update failed: #{e.class}: #{e.message}"
      )
      ...
      render json: {
        error: "Failed to update WireGuard route"
      }, status: :unprocessable_entity
    end
  end
  ...
  def destroy
    ...
    ip = @wireguard_peer.private_ip

    begin
      delete_route(ip) if ip.present?
    rescue => e
      Rails.logger.error(
        "Failed to delete WireGuard route #{ip}: #{e.message}"
      )
    end
...
def run_route_command(action, ip)
  ...
  def log_activity(action)
    ActivtyLog.create(
      ...
      ip: request.remote_ip,
      description:
        "#{action}d wireguard peer for private ip #{@wireguard_peer.private_ip}",
      user_agent: request.user_agent,
