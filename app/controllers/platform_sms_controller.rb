class PlatformSmsController < ApplicationController
  ...
  def balance
    ...
    if response.is_a?(Net::HTTPSuccess)
      ...
      Rails.logger.error "PlatformSmsController#balance: HTTP #{response.code} - #{response.body}"
    ...
    Rails.logger.error "PlatformSmsController#balance: unreachable (#{e.class}: #{e.message})"
    render json: { error: 'SMS provider unreachable' }, status: :service_unavailable
  rescue JSON::ParserError => e
    Rails.logger.error "PlatformSmsController#balance: bad response body: #{e.message}"
    render json: { error: 'Unexpected response from SMS provider' }, status: :service_unavailable
  rescue => e
    Rails.logger.error "PlatformSmsController#balance: #{e.class}: #{e.message}"
