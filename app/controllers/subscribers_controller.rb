...
#   use_autogenerated_number_as_ppoe_username = tenant_settings&.use_autogenerated_number_as_ppoe_username
#   use_autogenerated_number_as_ppoe_password = tenant_settings&.use_autogenerated_number_as_ppoe_password

#   prefix = tenant_settings&.prefix
#   minimum_digits = tenant_settings&.minimum_digits

#   unless prefix && minimum_digits
#     return render json: { error: 'Prefix and Minimum Digits not found' }, status: :not_found
#   end
...
#         key.to_s.downcase.strip.gsub(/\s+/, '_')
#       end.reject { |k, _| k.blank? }

#       normalized_row.delete("action")
#       normalized_row.delete("id")

#       subscriber = Subscriber.new(normalized_row)

#       # Generate ref_no with prefix + padded sequence number
#       if prefix && minimum_digits
#         "#{prefix}#{subscriber.sequence_number.to_s.rjust(minimum_digits.to_i, '0')}"
#       end
...
def import
  ...
  # ── count rows upfront so the frontend knows the total immediately ──
  total = count_import_rows(file)

  # ── copy temp file to a stable path the job can read after request ends ──
  job_id   = SecureRandom.hex(12)
  ext      = File.extname(file.original_filename)
  tmp_path = Rails.root.join('tmp', "import_#{job_id}#{ext}").to_s
  ...
  ActivtyLog.create(
    action:      'import',
    ip:          request.remote_ip,
    description: "Imported subscribers (job #{job_id})",
    user_agent:  request.user_agent,
    user:        current_user.username || current_user.email,
    date:        Time.current
  )
  ...
  render json: { job_id: job_id, total: total }, status: :ok
end
...
def registration_stats
  ...
  this_month_count = Subscriber.where(registration_date: Time.current.beginning_of_month..Time.current.end_of_month).count
  last_month_count = Subscriber.where(registration_date: 1.month.ago.beginning_of_month..1.month.ago.end_of_month).count
...
def create
  @subscriber = Subscriber.new(subscriber_params)
    ...
    @subscriber.update(registration_date: Time.current)
    @subscriber.update(date_registered: Time.current)
...
ActivtyLog.create(action: 'create', ip: request.remote_ip,
 description: "Created subscriber #{@subscriber.name}",
...
auto_generated_account_number = SecureRandom.random_number(10**4)
    ...
    auto_generated_no = "#{prefix}#{auto_generated_account_number.to_s.rjust(minimum_digits.to_i, '0')}" if prefix && minimum_digits
...
subscriber_welcome_message = ActsAsTenant.current_tenant&.subscriber_setting&.subscriber_welcome_message
if subscriber_welcome_message == true
...
def update 
    found_subscriber = set_subscriber
  unless found_subscriber
    return render json: { error: "Subscriber not found" }, status: :not_found
  end
  ...
  if found_subscriber.update(subscriber_params)
    ActivtyLog.create(
      action: 'update',
      ip: request.remote_ip,
      description: "Updated subscriber #{found_subscriber.name}",
      user_agent: request.user_agent,
      user: current_user.username || current_user.email,
      date: Time.current
    )
    found_subscriber.reload
    render json: found_subscriber, status: :ok
  else
...
company_name = ActsAsTenant.current_tenant&.company_setting&.company_name
    ...
    message: "Dear #{name}, Welcome to #{company_name || 'Aitechs'}. Your account has been created successfully",
        ...
        Rails.logger.info "SMS message sent: #{body}"
      ...
      Rails.logger.info "Failed to send message//eror response but message sent: #{response.body}"
  ...
  0
            ...
            def subscriber_params
              ...
              :house_number, :building_name, :latitude, :longitude,
               :email, :ppoe_package, :nas_router,
               
              :date_registered, :ref_no, :valid_until, :router_name, :package_name, :installation_fee, 
              :subscriber_discount, :second_phone_number,:node, location: [])
            end
