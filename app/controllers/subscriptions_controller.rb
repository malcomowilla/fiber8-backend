    ...
    if radacct
    ...
    else
      {
        ...
        # status: "Active - Never connected",
         status: subscription.status == 'blocked' ? 'blocked_offline' : 'offline_never_connected',
        last_seen: nil
      }
    end
  end
...
#       ssh.exec!("ip firewall address-list add list=aitechs_blocked_list address=#{params[:subscription][:ip_address]} comment=blocked_user_#{params[:subscription][:ppoe_username]}")
...
def block_service
  begin
    ...
    if ping_result
      ...
      Net::SSH.start(router.ip_address, router.username, password: router.password,
        ...
        ssh.exec!("ip firewall address-list add list=aitechs_blocked_list address=#{ip_address} comment=blocked_user_#{ppoe_username}")
        subscription.update!(status: 'blocked')

        render json: { message: "Blocked #{ppoe_username} (#{ip_address}) on MikroTik and updated status." }
      end
  ...
  subscription.update!(status: 'blocked')
      render json: { message: "#{ppoe_username} (#{ip_address}) is blocked" }
...
render json: @subscription, status: :created
  ...
  def destroy
    ...
    if @subscription.service_type == 'dhcp'
      ...
      RadReply.where(username: @subscription.mac_address).destroy_all
RadUserGroup.where(username: @subscription.mac_address, groupname: @subscription.package).destroy_all
RadCheck.where(username: @subscription.mac_address ).destroy_all
  ...
  #     # ✅ Delete the HotspotVoucher record
    ...
    if @subscription.service_type == 'pppoe'
      ...
      RadReply.where(username: @subscription.ppoe_username,
       ).destroy_all
  ...
  # user_group.update!(username: mac_or_identifier)
    ...
    def limit_bandwidth(ip_address, package, ppoe_username, subscriber_name)
      ...
      if response.is_a?(Net::HTTPSuccess)
        ...
        Rails.logger.info "Failed to  create subscription: #{response.code} - #{response.message}"
      end
    end
