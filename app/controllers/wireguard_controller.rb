...
class WireguardController < ApplicationController
  ...
  # POST /api/wireguard/generate_wireguard_app_config
  #
  # Standalone peer for a person's own device (phone/laptop) connecting
  # via the official WireGuard app — independent of the MikroTik
  # onboarding flow above. Unlike that flow, nobody pastes anything
  # into a router terminal here, so this endpoint has to apply the
  # peer to wg0 itself (previously it only wrote the WireguardPeer row
  # and relied on RehydrateWireguardJob's next run to pick it up,
  # which meant the tunnel could sit dead for a while after scanning
  # the QR code).
  def generate_wireguard_app_config
    host = request.headers['X-Subdomain']
    ...
    # Apply immediately so the app's handshake can succeed right away
    # instead of waiting on the next RehydrateWireguardJob pass.
    begin
      system("wg set wg0 peer #{client_public_key} allowed-ips #{random_ip}/32")
    rescue => e
      render json: { error: "Failed to apply peer to wg0: #{e.message}" }, status: :internal_server_error
