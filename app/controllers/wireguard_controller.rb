...
class WireguardController < ApplicationController
  ...
  def provision_router
    ...
    # This SSHes into the MikroTik router itself (over the WG tunnel),
    # not the AWS WireGuard host — left as-is.
    Net::SSH.start(router.ip_address, 'admin', password: '') do |ssh|
      ssh.exec!("/user add name=#{router.username} password=#{router.password} group=full")
      ssh.exec!("/tool fetch url=https://#{request.host}/api/mikrotik_callback address=#{router.ip_address}")
    end
  ...
  def generate_config
    ...
    # Generate client keypair + fetch server public key on the AWS box
    begin
      client_private_key, client_public_key = RemoteWireguardExecutor.generate_keypair
      server_public_key = RemoteWireguardExecutor.server_public_key
    rescue => e
      render json: { error: "Failed to generate WireGuard keys: #{e.message}" }, status: :internal_server_error
      return
    end
    ...
    # Apply peer on the AWS box
    begin
      RemoteWireguardExecutor.wg("set wg0 peer #{client_public_key} allowed-ips #{random_ip}/32")
    rescue => e
      render json: { error: "Failed to apply peer to wg0: #{e.message}" }, status: :internal_server_error
      return
    end
