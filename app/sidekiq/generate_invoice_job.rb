...
#     when "Talk Sasa"
#       send_expiration_talksasa(phone_number, due_date, invoice_number, username, plan_name, tenant)
#     else
...
#   def send_expiration_talksasa(phone_number, due_date, invoice_number, username, plan_name, tenant)
...
class GenerateInvoiceJob  < ApplicationJob
  ...
  # Returns true if the tenant has a valid hotspot/dial plan and its expiry is within 3 days
  def hotspot_and_dial_plan_billable?(tenant)
    plan = tenant.hotspot_and_dial_plan
    return false unless plan.present?
    return false unless plan.expiry.present?
  ...
  def generate_usage_invoice(tenant)
    ...
    # Calculate hotspot charge (tiered flat fee based on hotspot revenue)
    hotspot_total, hotspot_charge = calculate_hotspot_charge(tenant)

    # Calculate PPPoE charge (KES 10 per active client)
    pppoe_clients, pppoe_charge = calculate_pppoe_charge(tenant)
    ...
    if hotspot_charge > 0
      description_items << {
        description: "Hotspot platform fee",
        details: hotspot_fee_details(hotspot_total),
        quantity: hotspot_total,
        unit: "KES",
        rate: "Flat fee",
        amount: hotspot_charge,
    ...
    if pppoe_charge > 0
      description_items << {
        ...
        quantity: "#{pppoe_clients} clients",
        unit: "clients",
        rate: "#{PPPoE_PRICE_PER_CLIENT} KES/client",
        amount: pppoe_charge,
        currency: "KES"
      }
    end
  ...
  # Tiered hotspot fee:
  #   no hotspot revenue in the billing window     -> 0
  #   revenue up to HOTSPOT_HIGH_VOLUME_THRESHOLD   -> HOTSPOT_FEE_STANDARD (1000)
  #   revenue above HOTSPOT_HIGH_VOLUME_THRESHOLD   -> HOTSPOT_FEE_HIGH_VOLUME (2000)
  def calculate_hotspot_charge(tenant)
    start_date = tenant.last_billed_at || Time.current.beginning_of_month
    ...
    hotspot_charge =
      if hotspot_total > HOTSPOT_HIGH_VOLUME_THRESHOLD
        HOTSPOT_FEE_HIGH_VOLUME
      elsif hotspot_total > 0
        HOTSPOT_FEE_STANDARD
      else
        0
      end
  ...
  def hotspot_fee_details(hotspot_total)
    if hotspot_total > HOTSPOT_HIGH_VOLUME_THRESHOLD
      "High-volume hotspot fee (revenue above #{HOTSPOT_HIGH_VOLUME_THRESHOLD} KES this period)"
    else
      "Standard hotspot platform fee (charged when hotspot is active)"
    end
  end
