  ...
  def self.send_sms(
    ...
    if wallet.balance < 1
      SystemAdminSm.create!(
        user: phone_number,
        message: message,
        status: 'Failed: insufficient SMS balance',
      ...
      return {
        success: false,
        error: 'Insufficient SMS balance. Please top up to continue sending.'
      }
    end

    result = PlatformBulkSmsService.send_sms(
      phone_number,
      message,
      account_id,
      voucher
    ...
    # Only charge for sends that actually went out.
    if result[:success]
      voucher_ref =
        voucher.respond_to?(:voucher) ? "voucher:#{voucher.voucher}" : nil
