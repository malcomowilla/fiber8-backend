  ...
  class << self
    ...
    def batch_tcp_check(ip_port_pairs, timeout_sec = 2)
      ...
      ip_port_pairs.each do |ip, port|
        ...
        script << <<~SH
          start=$(date +%s%3N)
          if timeout #{timeout_sec} bash -c 'cat < /dev/null > /dev/tcp/#{ip}/#{port}' 2>/tmp/err_#{safe_ip}; then
            end_t=$(date +%s%3N)
            echo "#{ip}|OK|$((end_t-start))"
          else
            echo "#{ip}|FAIL|$(cat /tmp/err_#{safe_ip} 2>/dev/null | tr -d '\\n')"
          fi
          rm -f /tmp/err_#{safe_ip}
        SH
      end
      ...
      if results.empty?
        Rails.logger.error "[RemoteWireguardExecutor] batch_tcp_check returned no results for #{ip_port_pairs.size} target(s) — remote script likely failed"
      end
